...
- 환경변수 `PAPERCLIP_LOG_DIR`로 경로 변경 가능
...
```bash
# 가장 최신 로그 파일 찾기
ls -lt ~/.paperclip/instances/default/logs/ | head -3

# 실시간 에러 확인 (최신 파일 지정)
tail -f ~/.paperclip/instances/default/logs/server.5.log | jq 'select(.level >= 40)'

# 최근 에러만 보기
tail -500 ~/.paperclip/instances/default/logs/server.5.log | jq 'select(.level >= 40)'
...
- 접속 정보: `postgres://papercompany:papercompany@localhost:5432/papercompany_dev`
